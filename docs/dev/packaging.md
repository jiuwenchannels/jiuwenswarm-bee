...
Two installers are possible, one per desktop shell. The **web app must be built first** because both shells package `apps/web/dist`.
...
The `apps/desktop/electron/package.json` is already configured (`electron-builder`, `extraResources` copies `apps/web/dist` into the app, NSIS target).
...
| Output | Path | Size |
|---|---|---|
| Installer | `apps/desktop/electron/release/BeeChat Setup 0.1.0.exe` | ~82 MB |
| Unpacked app | `apps/desktop/electron/release/win-unpacked/BeeChat.exe` | — |
...
1. **Run the terminal as Administrator**, or turn on **Developer Mode** (Settings → System → For developers), then remove `"signAndEditExecutable": false` from `apps/desktop/electron/package.json` to get the exe icon/version too.
...
| Output | Path |
|---|---|
| NSIS installer | `apps/desktop/tauri/src-tauri/target/release/bundle/nsis/BeeChat_0.1.0_x64-setup.exe` |
| MSI | `apps/desktop/tauri/src-tauri/target/release/bundle/msi/BeeChat_0.1.0_x64_en-US.msi` |

Requires **Rust + MSVC C++ build tools** (see `apps/desktop/tauri/README.md`). Expect ~5 MB instead of ~82 MB, because it uses the OS WebView2 instead of bundling Chromium.
...
| Electron | `apps/desktop/electron/package.json` → `version` (used in the installer filename) |
...
|---|---|---|---|
| Electron | `cd apps/desktop/electron && npm run package` | `release/BeeChat Setup <v>.exe` | ~82 MB |
| Tauri | `cd apps/desktop/tauri && npm run icon && npm run build` | `src-tauri/target/release/bundle/**` | ~5 MB |
