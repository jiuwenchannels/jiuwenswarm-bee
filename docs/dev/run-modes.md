...
|---|---|---|
| **Web app** (the only real app) | `apps/web/` | Vite + React + TS: UI, chat, avatar, voice, gateway client. Builds to `apps/web/dist`. |
| **Electron shell** | `apps/desktop/electron/` | Native wrapper around the web app (bundled Chromium + Node). |
| **Tauri shell** | `apps/desktop/tauri/` | Native wrapper around the web app (Rust + OS WebView2), tiny. |

The shells contain almost no product code — they load `apps/web/dist` (or the web dev server) and only add OS behavior: transparent always-on-top window, tray, hotkeys, click-through, resize.

**Effective order:** change `apps/web/` → all three update. Touch `apps/desktop*/` only for OS/window behavior.
...
`apps/web/dist` is a plain static site (relative asset paths). Upload its contents to any web server / static host and open `index.html` (add `#avatar` for the avatar view).
...
npm install
npm start            # production: loads ../../web/dist
# or
...
| Loads | dev URL `http://localhost:5175` (dev) or `../web/dist` (build) |
...
| Show it to someone online | **C** (static host of `apps/web/dist`) |
...
| Gateway URL | default tries `ws://127.0.0.1:19000/ws` (product) then the SDK gateways; override in `apps/web/.env` with `VITE_JIUWENSWARM_URL` |
...
flowchart TD
    ...
    UI --> A["A · cd apps/web; npm run dev → :5175"]
    DESK --> D["D · cd apps/desktop/electron; npm start (Electron)"]
    DESK --> E["E · cd apps/desktop/tauri; npm run dev (Tauri)"]
    SHIP --> E2["E · npm run build → installer"]
    WEB --> B["B · npm run build + preview → :4173"]
    WEB --> C["C · host apps/web/dist"]
```
...
|---|---|---|---|
| A Web dev | `cd apps/web && npm run dev` | http://localhost:5175/ · `/#avatar` | Node |
| B Web preview | `cd apps/web && npm run build && npm run preview` | http://localhost:4173/ | Node |
| C Web static | host `apps/web/dist` | your host | any static host |
| D Electron | `cd apps/desktop/electron && npm start` | avatar window (tray/hotkeys) | Node |
| D' Electron dev | `cd apps/desktop/electron && npm run start:dev` | loads :5175 | Node + A running |
| E Tauri dev | `cd apps/desktop/tauri && npm run dev` | avatar window | Rust + MSVC |
| E' Tauri build | `cd apps/desktop/tauri && npm run build` | installer in `target/release/bundle` | Rust + MSVC |
