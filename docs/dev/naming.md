...
(`apps/web/src/i18n/`), so each audience gets the name that is best for it:
...
alternative — it is a one-line change in `apps/web/src/i18n/zh.ts`.
...
3. **The character name comes from the language pack** (`Buzz` / `嗡嗡`). Never hardcode
   it in a component; read it from `apps/web/src/i18n`.
