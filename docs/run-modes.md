...
| **Electron shell** | `desktop/` | Native wrapper around the web app (bundled Chromium + Node). |
...
flowchart TD
    ...
    DESK --> D["D · cd desktop; npm start (Electron)"]
...
| D Electron | `cd desktop && npm start` | avatar window (tray/hotkeys) | Node |
| D' Electron dev | `cd desktop && npm run start:dev` | loads :5175 | Node + A running |
