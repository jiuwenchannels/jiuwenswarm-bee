...
```powershell
cd C:\Workspace\openjiuwenchannels\jiuwenswarm-bee\bee
...
The `desktop/package.json` is already configured (`electron-builder`, `extraResources` copies `bee/dist` into the app, NSIS target).

```powershell
cd C:\Workspace\openjiuwenchannels\jiuwenswarm-bee\desktop
npm install
...
| Output | Path | Size |
|---|---|---|
| Installer | `desktop/release/BeeChat Setup 0.1.0.exe` | ~82 MB |
| Unpacked app | `desktop/release/win-unpacked/BeeChat.exe` | — |
...
```powershell
cd C:\Workspace\openjiuwenchannels\jiuwenswarm-bee\desktop-tauri
npm install
...
| Electron | `desktop/package.json` → `version` (used in the installer filename) |
| Tauri | `desktop-tauri/tauri.conf.json` → `version` |
...
|---|---|---|---|
| Electron | `cd desktop && npm run package` | `release/BeeChat Setup <v>.exe` | ~82 MB |
