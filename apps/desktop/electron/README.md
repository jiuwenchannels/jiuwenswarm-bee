# BeeChat desktop — Electron shell
...
An Electron shell around the existing web app — the avatar, chat, gateway client, and voice all live in `../../web`.
...
- A running JiuwenSwarm gateway (the web app auto-connects; see `../../web/.env`)
...
cd apps/web
npm install
npm run build            # produces apps/web/dist used by the shell
# ...or run the dev server: npm run dev

# 2. Run the desktop shell
cd ../desktop/electron
npm install
npm start                # loads apps/web/dist
...
| `build/icon.png` | electron-builder source icon (tray icon comes from the shared `apps/web/src/assets/`) |
| `../../web/src/components/avatar/*` | the character, inline chat, and voice |
