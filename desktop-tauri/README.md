...
BeeChat as a **Windows desktop assistant**, built with **Tauri v2** — a tiny native shell (Rust) around the same web app, rendered by the OS webview (WebView2). Same behaviour as the Electron shell in `../desktop/`, but the installer is a few MB instead of ~100 MB.
...
The avatar UI comes from `../bee/dist` (`index.html#avatar`).
...
```bash
npm run icon             # npx tauri icon ../bee/src/assets/bee-static.png
```
...
| Concern | Electron (`../desktop`) | Tauri (`here`) |
|---|---|---|
| Shell language | JavaScript (`main.js`) | Rust (`src-tauri/src/main.rs`) |
| Avatar UI | `bee/dist/index.html#avatar` | same (frontendDist) |
